...
class AgentState(BaseModel):
    ...
    analysis: Dict[str, Any] = Field(
        default_factory=dict, description="분석 결과 (감성, 키워드 등)"
    )
    ...
    metrics: Dict[str, float] = Field(
        default_factory=dict, description="품질 메트릭 (커버리지, 사실성, 실행 가능성)"
    )
...
class SocialTrendAgentState(AgentState):
    ...
    platforms: List[str] = Field(
        default_factory=lambda: ["x", "instagram", "naver_blog"], description="수집할 소셜 플랫폼"
    )
    ...
    engagement_stats: Dict[str, Any] = Field(
        default_factory=dict, description="플랫폼별 참여도 통계"
    )
