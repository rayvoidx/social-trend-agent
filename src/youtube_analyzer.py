...
class YouTubeAnalyzer:
    ...
    def analyze_subtitle_segments(
        ...
        try:
            ...
            if current_segment_text:
                segments.append({
                    "start_seconds": current_segment_start,
                    "end_seconds": end_seconds if 'end_seconds' in locals() else current_segment_start + segment_duration,
                    "duration": (end_seconds if 'end_seconds' in locals() else current_segment_start + segment_duration) - current_segment_start,
                    "text": ' '.join(current_segment_text),
                    "word_count": len(' '.join(current_segment_text).split())
                })
    ...
    def extract_keywords_from_segments(
        ...
        stop_words = {
            ...
            '이', '그', '저', '의', '을', '를', '에', '에서', '로', '으로', '와', '과', '는', '은', '가'
        }
