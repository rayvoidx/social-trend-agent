...
class HttpMCP:
    ...
    def __init__(self, timeout: float = 5.0, max_bytes: int = 200_000, verify_ssl: Optional[bool] = None):
            ...
            try:
                ...
                if self.verify_ssl:  # verify_ssl이 True였던 경우에만 재시도
                    try:
                        ...
                        logger.warning(f"HTTP MCP fetch failed for {url} (retry failed): {retry_err}")
                        return {"url": url, "error": f"SSL error and retry failed: {str(retry_err)}"}
                else:
...
class YouTubeMCP:
    ...
    def __init__(self, api_key: Optional[str] = None, timeout: float = 10.0, enable_analyzer: bool = True):
        """
        Args:
        ...
        if not api_key:
            logger.warning("YouTubeMCP: No API key provided. Set YOUTUBE_API_KEY environment variable.")
            return
        ...
        try:
            ...
            logger.error(f"YouTubeMCP: google-api-python-client not installed. Install with: pip install google-api-python-client. Error: {e}")
            self.youtube = None
        except Exception as e:
        ...
        try:
            ...
            if not channel_id:
                if channel_username:
                    ...
                    if not channel_id:
                        logger.warning(f"YouTubeMCP: Channel not found for username/handle: {channel_username}")
                        return []
        ...
        try:
            ...
            try:
                ...
                if response.get("items"):
                    ...
                    logger.info(f"YouTubeMCP: Found channel ID {channel_id} for handle @{handle_clean}")
                    return channel_id
            ...
            try:
                ...
                if search_response.get("items"):
                    ...
                    logger.info(f"YouTubeMCP: Found channel ID {channel_id} for handle @{handle_clean} via search")
                    return channel_id
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    chapters.append({
                        ...
                        "timecode": f"{hours:02d}:{minutes:02d}:{seconds:02d}" if hours > 0 else f"{minutes:02d}:{seconds:02d}",
                        "title": title,
                        "source": "description"
                    })
                    break
    ...
    async def fetch_for_rag(
        ...
        if not target_channel_id and channel_handle:
            ...
            if found_channel_id:
                ...
                logger.info(f"YouTubeMCP: Resolved channel handle @{handle_clean} to channel ID {target_channel_id}")
            else:
                logger.warning(f"YouTubeMCP: Failed to resolve channel handle @{handle_clean} to channel ID")
        ...
        if target_channel_id:
            try:
                ...
                if videos:
                    logger.info(f"YouTubeMCP: Fetched {len(videos)} videos from channel {target_channel_id} for query: {query}")
                    # 채널 정보 가져오기
                    ...
                    logger.info(f"YouTubeMCP: No videos found in channel {target_channel_id} for query: {query}, fetching latest videos")
                    ...
                    try:
                        ...
                        if latest_videos:
                            logger.info(f"YouTubeMCP: Fetched {len(latest_videos)} latest videos from channel {target_channel_id}")
                            channel_info = self._get_channel_info(target_channel_id)
                            return {
                            ...
                            logger.warning(f"YouTubeMCP: No videos available in channel {target_channel_id}")
                    except Exception as e2:
                        logger.error(f"YouTubeMCP: Failed to get latest videos: {e2}", exc_info=True)
            except Exception as e:
                logger.warning(f"YouTubeMCP: Failed to search channel videos: {e}")
        
        # 채널 ID가 없으면 YouTube 관련 키워드가 있을 때만 일반 검색
        youtube_keywords = [
            '유튜브', 'youtube', '영상', '동영상', '비디오', 'video', 
            '펭수', '펭', '채널', 'channel', '놀이', '배우', '시청', 
            '콘텐츠', 'content', '미디어', 'media', '강의', '교육영상'
        ]
        ...
        if has_youtube_keyword:
            ...
            logger.debug(f"YouTubeMCP: No YouTube-related keywords in query and no channel ID set: {query}")
