            ...
            for attempt in range(dyn_retries + 1):
                try:
                    if isinstance(plan_timeout, int) and plan_timeout > 0:
                        return await asyncio.wait_for(func(*args, **kwargs), timeout=float(plan_timeout))
                    return await func(*args, **kwargs)
                except exceptions as e:
                    last_exception = e
                    if attempt == dyn_retries:
                        logger.error(f"Function {func.__name__} failed after {dyn_retries} retries. Last error: {e}")
