...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/mcp_server.log"), logging.StreamHandler(sys.stderr)],
)
logger = logging.getLogger(__name__)


def run_news_trend_analysis(
    ...
    try:
        ...
        result = run_agent(
            query=query, time_window=time_window, language=language, max_results=max_results
        )
        ...
        return {
            ...
            "summary": (
                analysis.get("summary", "분석 완료") if isinstance(analysis, dict) else "분석 완료"
            ),
            "sentiment": {
            ...
            "top_keywords": (
                keywords.get("top_keywords", [])[:5] if isinstance(keywords, dict) else []
            ),
            "total_items": len(getattr(result, "normalized", [])),
            "metrics": getattr(result, "metrics", {}),
            "report_preview": (
                (getattr(result, "report_md", "")[:500] + "...")
...
def run_viral_video_analysis(
    ...
    try:
        ...
        logger.info(
            f"Running viral video analysis: query={query}, market={market}, platforms={platforms}"
        )
        ...
        return {
            ...
            "platforms": platforms,
            "spikes_detected": (
                viral_data.get("total_spikes", 0) if isinstance(viral_data, dict) else 0
            ),
            "top_videos": top_videos,
            "success_factors": (
                analysis.get("success_factors", [])[:3] if isinstance(analysis, dict) else []
            ),
    ...
    try:
        ...
        for agent_dir in search_dirs:
            ...
            for md_file in sorted(
                agent_path.glob("*.md"), key=lambda x: x.stat().st_mtime, reverse=True
            )[:limit]:
                reports.append(
                    {
                        "agent": agent_dir,
        ...
        return {"success": True, "total": len(reports), "reports": reports[:limit]}
...
def get_agent_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "statistics": {"total_reports": total_reports, "artifacts_dir": str(artifacts_dir)},
            "agents": {"news_trend_agent": "operational", "viral_video_agent": "operational"},
        }
            ...
            if method == "tools/list":
                ...
                response = {
                    ...
                    "result": {
                        ...
                        "capabilities": {"tools": {}},
                        "serverInfo": {"name": "trend-analysis-agent", "version": "1.0.0"},
                    },
                }
