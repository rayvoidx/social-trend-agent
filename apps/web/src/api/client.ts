...
import type { AnalysisRequest, TaskStatus, StreamEventData } from "../types";
...
export const api = {
  ...
  async webSearch(query: string, topK: number = 5) {
    const response = await apiClient.post("/api/mcp/web-search", {
      query,
      top_k: topK,
    });
    return response.data;
  ...
  async recommendMissions(insightId: string) {
    const response = await apiClient.post("/api/mcp/recommend-missions", {
