...
def run_viral_video_agent(args):
    ...
    try:
        final_state = run_agent(
            query=args.query, market=args.market, platforms=platforms, time_window=args.window
        )
...
def send_n8n_notification(final_state):
    ...
    try:
        ...
        print("✅ n8n notification sent successfully")
    except Exception as e:
...
def send_slack_notification(final_state):
    ...
    message = {
        ...
        "blocks": [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": f"📊 Analysis: {final_state.get('query')}"},
            },
            {
                "type": "section",
                "fields": [
...
Examples:
  ...
  python scripts/run_agent.py --agent news_trend_agent --query "AI trends" --notify n8n,slack --emit md,json
    ...
    parser.add_argument(
        "--window", default="7d", help="Time window (e.g., 24h, 7d, 30d) [default: 7d]"
    )
    parser.add_argument(
        "--emit", default="md", help="Output format: md, json, or md,json [default: md]"
    )
    parser.add_argument(
        "--notify", default=None, help="Notification channels: n8n, slack, or n8n,slack"
    )

    # News trend agent specific
    parser.add_argument(
        "--language", default="ko", help="Language for news search (ko, en) [default: ko]"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--market", default="KR", help="Market code (KR, US, JP, etc) [default: KR]"
    )
    parser.add_argument(
        "--platform",
        default="youtube",
